...
void* msgHndlr(void *s) {
	...
	while (true) {
		...
		if (incoming == endc) {
			...
			for (auto it = allSkts.begin(); it != allSkts.end(); ++it) {
				if (*it == *clientSokt) {
					allSkts.erase(it);
					break; // Socket found and removed, exit loop
				}
			}

			cout <<  allSkts.size() << endl;
...
int main() {
	...
	try {
		...
		while (true) {
			...
			SOCKET clientSokt = accept(sevrSocket, (sockaddr*) &client,
					&cliSize);
			if (clientSokt == -1) {
				break;
			}
